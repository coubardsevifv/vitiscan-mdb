...
import { Parcelle, Placette, Emplacement, CategorieNotation, Notation, Prospection } from "@/api/entities";
...
export default function useSaisie(parcelleId,placetteId){
 ...
 const save=async(code)=>{if(!current)return;setSaving(true);const user=await getCurrentUser();let pros=(await Prospection.filter({parcelle_id:parcelleId,annee:YEAR}))[0];if(!pros&&navigator.onLine)pros=await Prospection.create({parcelle_id:parcelleId,annee:YEAR,statut:"en_cours",date_debut:new Date().toISOString(),utilisateur_id:user.id,utilisateur_nom:user.full_name});const payload={prospection_id:pros?.id||`offline-${parcelleId}-${YEAR}`,parcelle_id:parcelleId,placette_id:placetteId,emplacement_id:current.id,numero_emplacement:current.numero,rang:data.placette.rang,annee:YEAR,code,utilisateur_id:user.id,utilisateur_nom:user.full_name,date_saisie:new Date().toISOString(),hors_ligne:!navigator.onLine};const key=`${YEAR}-${current.id}`,queue=pending().filter(x=>x.key!==key);store([...queue,{key,id:currentNotation?.id,payload}]);setData(d=>({...d,notations:[...d.notations.filter(n=>n.emplacement_id!==current.id),payload]}));if(navigator.onLine){try{const saved=currentNotation?.id?await Notation.update(currentNotation.id,payload):await Notation.create(payload);store(pending().filter(x=>x.key!==key));setData(d=>({...d,notations:d.notations.map(n=>n.emplacement_id===current.id?saved:n)}));}catch{}}
